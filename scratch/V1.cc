...
NS_LOG_COMPONENT_DEFINE("SearchRescueSimulation_scenariofinalv1");
...
#include "../helperfiles/SAR_operations_logs/discoverynobuildings.cc"
...
int main(int argc, char *argv[]) {
    LogComponentEnable("SearchRescueSimulation_scenariofinalv1", LOG_LEVEL_INFO);
    ...
    rescueAssignmentLogFile.open("rescue_assignment_logfinalv1.csv");
    rescueAssignmentLogFile << "Timestamp,Civilian ID,Assigned SAR Node ID, Assigned SAR Node IP,Civilian X,Civilian Y,Civilian Z\n";
    receivedAssignmentsLogFile.open("recieved_assignment_logfinalv1.csv");
   ...
   signalLogFile.open("Civiliansignallogfinalv1.csv");
    ...
    movementLogFile.open("movemnetsfinalv1.csv");
    ...
    rescueCompletionLogFile.open("rescue_completion_logfinalv1.csv");
    ...
    routingLogFile.open("routing-table-logfinalv1.csv");
    // Add headers if necessary
    systemLogFile.open("system_events_logfinalv1.csv", std::ios::out | std::ios::trunc);
...
if (!macTxRxLogFile.is_open()) {
   ...
   attachmentLogFile.open("attachment_logfinalv1.csv");
    ...
    connectionLogFile.open("connection_status_logfinalv1.csv");
    connectionLogFile << "Time,Status" << std::endl;  
    gpsLogFile.open("gps_communication_logfinalv1.csv");
    gpsLogFile << "Time,Action,Type,NodeName,NodeAddress,OldX,OldY,OldZ,CurrentX,CurrentY,CurrentZ,Details" << std::endl;  
    videoLogFile.open("video_communication_logfinalv1.csv");
    videoLogFile << "Time,Action,Type,Source,Destination,Size (bytes)" << std::endl;  
    SetupLocalDiscoveryLogging();       
    SetupCentralDiscoveryLogging();     
    sarDiscoveryBroadcastLogFile.open("sar_discovery_broadcast_logfinalv1.csv");
    ...
    // creating nodes
    NodeContainer baseNode, droneNode,helicopterNode, vehicleNode, footNodes,civilianNodesPLB,civilianNodeswifi, civilianNodespassive;
    baseNode.Create(1);
    droneNode.Create(1);
...
for (uint32_t i = 0; i < allNodes.GetN(); i++) {
  ...
  // After creating nodes and assigning them to containers:
   ...
   // Install WiFi devices on foot teams, civilians, and vehicles
...
for (uint32_t i = 0; i < civilianNodesPLB.GetN(); i++) {
  ...
  Ipv4ListRoutingHelper list;
   ...
   internet.Install(allNodes);
    ...
    g_phyLogStream = Create<OutputStreamWrapper>("SNR_log_finalv1.csv", std::ios::out);
...
Simulator::Schedule(
        ...
        std::ofstream flowMonitorCsvFile(routingsize+","+std::to_string(runNumber)+"overall-flow-monitor-resultsfinalv1.csv", std::ios::app);
        flowMonitorCsvFile << "seed, runnum, routing, Flow ID,Start Time (s),End Time (s),Source IP,Destination IP,Source Node ID,Destination Node ID,Source Port, Destination Port, Tx Packets,Tx Bytes,"
        ...
        std::ofstream netPreformanceDataset(routingsize+","+std::to_string(runNumber)+"NetwrokDatasetfinalv1.csv", std::ios::app);
            netPreformanceDataset << "seed, runnum, routing,Flow ID,Start Time (s),End Time (s),Source IP,Destination IP,Source Node ID,Destination Node ID,Source Port,Destination Port,Protocol, Tx Packets,Tx Bytes,"
                        << "Rx Packets,Rx Bytes,Throughput (Mbps),Throughput (Kbps),Average Delay (s),Average Delay (ms),"
                        << "Average Jitter (s),Lost Packets,Packet Loss Ratio (%),Packet Delivery Ratio (%),Data Packet Delivery Ratio (%),"
                        << "Mean Hop Count,Packets Forwarded,Tx Rate (Kbps),Rx Rate (Kbps)\n";
...
//start routing files
 ...
 // Simulator::Stop(Seconds(20));
    ...
    Simulator::Run();
    RoutingTelemetry::Stop();         
    FinalizeEnergyLogs(nodeEnergySources);
    LogAverageSnrRssi();
    LogAverageSnrRssiNodes();
    for (auto& [nodeId, nodeInfo] : nodeDataset) {
        auto it = energyDataset.find(nodeId);
        if (it != energyDataset.end()) {
            nodeInfo.backupEnergy = it->second.residualEnergy;

            NS_LOG_UNCOND("Node " << nodeId 
                           << " backup energy set to residual: " 
                           << nodeInfo.backupEnergy << " J");
        } else {
            NS_LOG_WARN("Energy metrics not found for node " << nodeId 
                          << " when updating backup energy.");
        }
    }

    for (auto& entry : nodeDataset) {
        uint32_t nodeId = entry.first;
        NodeInfo& nodeInfo = entry.second;
    
        auto energyIt = energyDataset.find(nodeId);
        if (energyIt != energyDataset.end()) {
            const EnergyMetrics& metrics = energyIt->second;
            nodeInfo.backupEnergy = metrics.residualEnergy;
        }
    }
    
    FinalizeAllCommunicationSessions();
    WriteDiscoveryConvergenceDataset(routingsize+"_discovery_convergence_datasetfinalv1.csv");
...
std::ofstream outFile("OVERALL_network_statisticsfinalv1.csv", std::ios::app);
std::ifstream checkFile("OVERALL_network_statisticsfinalv1.csv");
...
WriteNodeInfoDataset("node_information_datasetfinalv1.csv");
  ...
  for (auto iter = flowMonitor->GetFlowStats().begin(); iter != flowMonitor->GetFlowStats().end(); ++iter)
  {
