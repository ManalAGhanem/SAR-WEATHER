...
// which internally calls the Vector-based version:
    ...
    Ptr<WeatherManager> weatherManager) 
{
    ...
    if (!weatherManager) {
        NS_LOG_WARN("⚠️ [WeatherManager] GetEffectiveLOSRange() called with nullptr!");
        return nominalRange;
    }

    //  Log the nominal range before applying weather effects
    //NS_LOG_INFO("🌍 Nominal Discovery Radius for Node " << node->GetId() << ": " << nominalRange << "m");

    // Apply weather attenuation
    double adjustedRange = weatherManager->GetEffectiveLOSRange(nominalRange);

    //  Log the adjusted range after weather effects
   // NS_LOG_INFO("⛈️ Adjusted Discovery Radius for Node " << node->GetId() << ": " << adjustedRange << "m");
...
//schedule weather chnages:
void UpdateWeather(Ptr<WeatherManager> weather, Ptr<WeatherAttenuationModel> weatherLoss) {
    Ptr<UniformRandomVariable> uv = CreateObject<UniformRandomVariable>();

    double rainRate = uv->GetValue(0.0, 70.0);
    double fogDensity = uv->GetValue(0.0, 40.0);
    double snowRate = uv->GetValue(0.0, 20.0);
    double temperature = uv->GetValue(10.0, 25.0);
    double windSpeed = uv->GetValue(10.0, 100.0);

    double baseTime = Simulator::Now().GetSeconds();
    double updateInterval = 3.0;  // How often weather updates happen
    double timeOffset = baseTime; // Use absolute increasing time

    //  Spread out updates over time
    weather->ScheduleWeatherChange(timeOffset + 0.1, "RainRate", rainRate);
    weather->ScheduleWeatherChange(timeOffset + 0.3, "FogDensity", fogDensity);
    weather->ScheduleWeatherChange(timeOffset + 0.5, "SnowRate", snowRate);
    weather->ScheduleWeatherChange(timeOffset + 0.7, "WindSpeed", windSpeed);
    Simulator::Schedule(Seconds(timeOffset + 0.9), &WeatherAttenuationModel::SetTemperature, weatherLoss, temperature);

    //NS_LOG_INFO(" Weather scheduled for update at " << baseTime
                // << ": RainRate=" << rainRate << " mm/h, FogDensity=" << fogDensity 
                // << " g/m³, SnowRate=" << snowRate << " mm/h, WindSpeed=" << windSpeed
                // << " m/s, Temperature=" << temperature << "°C");

    //  Schedule the next weather update using increasing time
    Simulator::Schedule(Seconds(updateInterval + baseTime), &UpdateWeather, weather, weatherLoss);

}

Vector GetBaseStationPosition() {
    // 1️⃣ First, check if a global base station pointer is available
    if (g_baseNode) {
    ...
    // 2️⃣ Second, check if we have a predefined base station Node ID
    ...
    // 3️⃣ Third, try to find the base station dynamically using its IP address
