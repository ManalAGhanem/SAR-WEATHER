...
void AssignMaxBandwidthToNodes(const NodeContainer& nodes, std::string dataRate)
{
...
void PopulateIpToNodeIdMap(const NodeContainer& nodes) {
    for (uint32_t i = 0; i < nodes.GetN(); ++i) {
...
//     std::ofstream debugFile("connectivity_debugfinalv3.log");
...
// Robust line-segment (src->dest) vs AABB (building bounds) test using the slab method.
// Keeps the same signature and behavior: returns true if *any* building blocks LOS,
// and fills blockingBuildingPos with that building's center.
void PositionChangeCallback(std::string context, Ptr<const MobilityModel> mobility) {
    ...
    auto itInit = initialPosRecorded.find(nodeId);
    if (itInit == initialPosRecorded.end() || !itInit->second)
    {
        initialPositions[nodeId] = position;
        initialPosRecorded[nodeId] = true;
    }
    ...
    if (positionLogFile.is_open()) {
        ...
        NS_LOG_ERROR("Could not open position_logfinalv3.csv for writing.");
    }
}
...
//     // Open a debug log file in append mode
//     // std::ofstream debugFile("connectivity_debugsfinalv3.log");
//     // if (!debugFile.is_open()) {
//     //     std::cerr << "Error: Could not open debug log file." << std::endl;
//     // } else {
...
void InitializeNodeDeviceAttributes(NodeInfo& nodeInfo) {
    if (nodeInfo.nodeDevice == "Handheld Radio") {
        ...
        nodeInfo.transmitPower = 15.0;          // dBm
        nodeInfo.receiverSensitivity = -85.0;   // dBm
        nodeInfo.antennaGainTx = 2.0;           // dB
        nodeInfo.antennaGainRx = 2.0;           // dB
        nodeInfo.initialEnergy = 3000.0;
        nodeInfo.backupEnergy  = 1500.0;

    } else if (nodeInfo.nodeDevice == "Onboard WiFi") {
        nodeInfo.pathLossModel = "Friis";
        nodeInfo.transmitPower = 20.0;          // dBm
        nodeInfo.receiverSensitivity = -90.0;   // dBm
        nodeInfo.antennaGainTx = 3.0;           // dB
        nodeInfo.antennaGainRx = 3.0;           // dB
        nodeInfo.initialEnergy = 80000.0;
        nodeInfo.backupEnergy  = 40000.0;

    } else if (nodeInfo.nodeDevice == "Aerial WiFi") {
        nodeInfo.pathLossModel = "Friis";
        nodeInfo.transmitPower = 23.0;          // was 25.0 (reduce EIRP)
        nodeInfo.receiverSensitivity = -88.0;//-88
        nodeInfo.antennaGainTx = 5.0;
        nodeInfo.antennaGainRx = 5.0;
        nodeInfo.initialEnergy = 15000.0;
        nodeInfo.backupEnergy  = 7500.0;

    } else if (nodeInfo.nodeDevice == "High-Gain Relay") {
        nodeInfo.pathLossModel = "Friis";
        nodeInfo.transmitPower = 24.0;          // was 30.0 (avoid multi-watt EIRP)
        nodeInfo.receiverSensitivity = -93.0;   // -93
        nodeInfo.antennaGainTx = 5.0;           // was 8.0
        nodeInfo.antennaGainRx = 6.0;           // was 8.0
        nodeInfo.initialEnergy = 200000.0;
        nodeInfo.backupEnergy  = 100000.0;

    } else if (nodeInfo.nodeDevice == "Fixed High-Gain") {
        nodeInfo.pathLossModel = "Friis";
        nodeInfo.transmitPower = 23.0;          // was 30.0
        nodeInfo.receiverSensitivity = -93.0;   // -93
        nodeInfo.antennaGainTx = 4.0;           // was 10.0  -> avoids 10W (40 dBm) EIRP
        nodeInfo.antennaGainRx = 6.0;           // was 10.0
        // (If you want, set initialEnergy/backupEnergy here too, like your other types.)

    } else if (nodeInfo.nodeDevice == "PLB") {
        nodeInfo.pathLossModel = "Friis";
        nodeInfo.transmitPower = 14.0;          // was 20.0 (more realistic beacon power)
...
//         nodeInfo.antennaGainTx = 2.0;
//         nodeInfo.antennaGainRx = 2.0;
//         nodeInfo.initialEnergy = 3000.0;   // 3,000 Joules
//          nodeInfo.backupEnergy = 1500.0;
//     } else if (nodeInfo.nodeDevice == "Onboard WiFi") {
//         nodeInfo.pathLossModel = "Friis";
...
//         nodeInfo.antennaGainRx = 2.0;
//         nodeInfo.initialEnergy = 3000.0;   // 3,000 Joules
//          nodeInfo.backupEnergy = 1500.0;   // 50% of primary
//     }else if (nodeInfo.nodeDevice == "Hybrid msgs reciever") {
//         nodeInfo.pathLossModel = "Friis";
//         nodeInfo.transmitPower = 15.0;
//         nodeInfo.receiverSensitivity = -90.0;
...
{
    //std::ofstream logFile("WiFiConfigurationLogfinalv3.txt", std::ios::app); // Open log file in append mode
...
//     std::ofstream out("MacTxRxLogfinalv3.txt");
...
void WriteNodeInfoDataset(const std::string& filename) {
    std::ofstream file(filename);
     ...
     if (!file.is_open())
    {
        std::cerr << "Error: Could not open node info dataset file: "
                  << filename << std::endl;
        return;
    }
    file << "NodeID,IP Address,NodeType,NodeRole,Device,Primary Battery Capacitiy, Backup Battery Capacity,TransmitPower,ReceiverSensitivity,MaxBandwidth(Mbps),Channel Bandwidth (MHz),Noise,Path Loss Model,antennaGainTx ,antennaGainRx,InitialX,InitialY,InitialZ,FinalX,FinalY,FinalZ,"
         << "MobilityModel,Speed KM/H,CustomAttributes\n";
...
//     std::ofstream file(filename);
//     file << "NodeID,IP Address,NodeType,NodeRole,Device,Primary Battery Capacitiy, Backup Battery Capacity,TransmitPower,ReceiverSensitivity,MaxBandwidth(Mbps),Channel Bandwidth (MHz),Noise,Path Loss Model,antennaGainTx ,antennaGainRx,InitialX,InitialY,InitialZ,FinalX,FinalY,FinalZ,"
//          << "MobilityModel,Speed KM/H,ConnectivityLevel,CommunicationRange (KM),CustomAttributes\n";
