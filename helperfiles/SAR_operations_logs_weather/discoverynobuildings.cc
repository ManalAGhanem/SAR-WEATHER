...
if (    // 2) Civilian is in g_civilianAttachedState AND is attached (== true)
  ...
  if (signalType.empty()) {
    ...
    // double dx = position.x - lastPos.x;
    // double dy = position.y - lastPos.y;
    // double dz = position.z - lastPos.z;
    // double distMoved = std::sqrt(dx*dx + dy*dy + dz*dz);

   //double locationChangeThreshold = 1.0; // Only log again if it moved >1 meter

   // If already discovered and the position changed < 1 meter, skip repeated log
    // if (discoveryState[sarNodeId][civilianNodeId] && distMoved < locationChangeThreshold) {
    ...
    //     // We do not re-log or re-discover. Just skip.
    //   //  return;
    // }
   ...
   // Ptr<Node> sarNode = NodeList::GetNode(sarNodeId);
    ...
    uint32_t communicationIddiscovery = 11000 + sarNodeId;
      ...
      sendDiscoveryToBase(
        ...
        // double fallbackTimeout = 3.0; // Time limit before checking for self-assignment
        // Simulator::Schedule(Seconds(fallbackTimeout), &CheckAssignmentStatusAndPossiblySelfAssign, 
        // sarNodeId, civilianNodeId, baseStation);
    // Generate a unique communication ID for the broadcast
  //  uint32_t communicationId = 6000 + sarNodeId;
...
// auto itSar = discoveryState.find(sarNodeId);
// if (itSar != discoveryState.end())
// {
//     auto itCiv = itSar->second.find(civilianNodeId);
//     if (itCiv != itSar->second.end())
//         alreadyDiscovered = itCiv->second;
// }

// if (alreadyDiscovered && distMoved < locationChangeThreshold)
// {
//     discoveryLogFile << Simulator::Now().GetSeconds() << ",SAR Node " << sarNodeId
//                      << ",discovered Civilian Node " << civilianNodeId
//                      << ", via " << signalType
...
//     // Check if this SAR node has already discovered this civilian
//     // Use SAR → Civilian discoveryState ONLY
// auto &sarMap = discoveryState[sarNodeId];

// if (sarMap.find(civilianNodeId) == sarMap.end())
// {
//     sarMap[civilianNodeId] = true;

//     // Civilian-centric tracking stays separate
//     g_civilianToDiscoveringNodesCount[civilianNodeId]++;
//     g_civilianToDiscoveringNodeIps[civilianNodeId].insert(sarIp);
// }
...
//     //     allDiscoveringNodeIds.push_back(entry.first);
//     // }
//     for (const auto &[sarId, civMap] : discoveryState) {
//     if (civMap.find(civilianNodeId) != civMap.end()) {
//         allDiscoveringNodeIds.push_back(sarId);
//     }
// }
...
// Helper: nominal (clear-weather) discovery radius by node type.
// No dependency on WeatherManager.
    ...
    double thermalRange = GetThermalImagingRange(sarNode, droneNodes, vehicleNodes, footNodes, helicopterNodes);
     ...
     Ptr<Socket> discoverySocket = Socket::CreateSocket(sarNode, UdpSocketFactory::GetTypeId());
        ...
        for (uint32_t j = 0; j < civilianNodes.GetN(); ++j) {
           ...
           // if (discoveryState[sarId][civId]) continue;
...
void PerformPassiveDiscoveryForUndiscovered(
    ...
    droneNodes, vehicleNodes, footNodes, helicopterNodes,  weatherManager, allBuildings
...
void DiscoverCivilianNodes(
 ...
 //   AnimationInterface* anim,
    ...
    Simulator::Schedule(Seconds(3.0), &DiscoverCivilianNodes,
                        ...
                        footNodes, helicopterNodes, weatherManager, allBuildings,trees);
}
                       ...
                       ) // rename param
{
   ...
   // uint32_t iComm = 0; // offset to add to baseCommunicationId
    ...
    for (uint32_t i = 0; i < sarNodes.GetN(); ++i) {
        ...
        // // Unique comm ID for each receiver
        // uint32_t commIdThisReceiver = baseCommunicationId + iComm;
        // iComm++;
    ...
    if (sessionTable.find(sessionKey) != sessionTable.end()) {
        ...
        // Send packet
         Time extraJitter = MilliSeconds(NodeList::GetNode(senderNode->GetId())->GetId() * 10 + (rand() % 100));

    // Schedule the actual send instead of calling it directly
        //  Simulator::Schedule(extraJitter, &Socket::SendTo, sendSocket, packet, 0, remote);
       // sendSocket->SendTo(packet, 0, remote);
Simulator::Schedule(extraJitter,
  [sendSocket, packet, remote]() {
    sendSocket->SendTo(packet, 0, remote);
  });
        // Log
        LogPacketEvent(
...
// // Function to retrieve and log civilian node's position
// void LogCivilianPosition(Ptr<Node> civilianNode, std::ofstream& logFile) {
//     Ptr<MobilityModel> mobility = civilianNode->GetObject<MobilityModel>();
//     if (mobility) {
//         Vector position = mobility->GetPosition();
//         logFile << position.x << "," << position.y << "," << position.z;
//     } else {
//         logFile << "Error: MobilityModel not found";
//     }
// }
...
void  sendDiscoveryToBase(uint32_t sarNodeId, uint32_t civilianNodeId, double distance, bool inRange,
    ...
    Time sendDelay = MilliSeconds(1 + (rand() % 500)); 
    

    // Schedule the socket send
  //  Simulator::Schedule(sendDelay, &Socket::SendTo, sendSocket, packet, 0, remote);
...
void ReceiveBroadcast(Ptr<Socket> socket) {
    ...
    Address from;
    Ptr<Packet> packet = socket->RecvFrom(from);
 //std::cout << "[RX_HANDLER] t=" << Simulator::Now().GetSeconds()
        //   << " node=" << socket->GetNode()->GetId()
        //   << " handler=SAR RECIEVE DISCOVERY BROADCAST Received" << std::endl;
...
void ProcessDiscoveryPacket(Ptr<Socket> socket, const NodeContainer& sarNodes, Ptr<Socket> baseBroadcastSocket, uint16_t discoverybasePort) {
    ...
    //  std::cout << "[RX_HANDLER] t=" << Simulator::Now().GetSeconds()
    //       << " node=" << socket->GetNode()->GetId()
    //       << " handler=DISCOVERY PACKETS Received AT STATION" << std::endl;
    ...
    // ---- SEND ACK BACK TO SAR ----
      ...
      //  socket->SendTo(ackPacket, 0, sarAddr);
       ...
       // schedule ACK instead of immediate send
        Ptr<Socket> s = socket;                // keep Ptr alive
        Ptr<Packet> p = ackPacket->Copy();     // safe copy
        Address a = sarAddr;                   // destination

        Simulator::Schedule(MilliSeconds(3), [s, p, a]() {
            s->SendTo(p, 0, a);
        });
...
Simulator::Schedule(MilliSeconds(3),&AssignRescuer,data.civilianNodeId, civilianNode, sarNodes, baseBroadcastSocket, assignmentPort);
