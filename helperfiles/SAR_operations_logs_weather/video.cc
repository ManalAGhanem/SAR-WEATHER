...
//     // }
// //uint32_t communicationId = ipToCommIdMap[senderIp];
//     LogPacketEvent(
//         socket,
//         receiverAddress,                 // Source IP
...
//         packet->GetSize(),        // Packet size
//         videoPort,
//         "Video Packet Recieved",           // Packet type
//         communicationId           // Communication ID
//     );
//     videoLogFile << receiveTime << ",Received, Video," << address.GetIpv4() << "," << receiverAddress << ","
//                  << packet->GetSize() << " bytes" << std::endl;

//     // Log packet event with calculated delay and throughput
//    // LogPacketEvent(socket, address.GetIpv4(), receiverAddress, packetId, delay, throughput, packet->GetSize(),"VIDEO");
...
uint32_t senderNodeId = node->GetId();
  ...
  // Update session metrics
    ...
    //LogPacketEvent(socket, senderAddress, receiverAddress, packetId, 0.0, 0.0, packet->GetSize(),"VIDEO");  // Placeholder for delay, throughput
