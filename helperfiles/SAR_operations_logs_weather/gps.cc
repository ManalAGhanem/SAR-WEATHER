...
void LogGPSData(Ptr<Socket> socket)
{
    ...
    while (Ptr<Packet> packet = socket->RecvFrom(from))
    {
        ...
        // If you truly require this tag, you can 'continue' if missing,
        // but don't return (return would drop other queued packets).
        // else { continue; }

        // Extract the NodeIdTag (if present)
        // NodeIdTag nodeIdTag;
        // uint32_t senderNodeId = 0;
        // if (packet->PeekPacketTag(nodeIdTag))
        // {
        //     senderNodeId = nodeIdTag.GetNodeId();
        // }
        ...
        // IMPORTANT: source/destination order
        LogPacketEvent(
            socket,
            senderIp,               // Source IP (sender)
            receiverIp,             // Destination IP (receiver/base)
            packetId,
            delay,
            throughput,
            packet->GetSize(),
...
//     {
//         InetSocketAddress address = InetSocketAddress::ConvertFrom(from);
//         Ipv4Address baseStationAddress =
//             socket->GetNode()->GetObject<Ipv4>()->GetAddress(1, 0).GetLocal();
...
//     // InetSocketAddress address = InetSocketAddress::ConvertFrom(from);
//     // Ipv4Address baseStationAddress = socket->GetNode()->GetObject<Ipv4>()->GetAddress(1, 0).GetLocal();
//     // uint32_t packetId = packet->GetUid();
//     // double receiveTime = Simulator::Now().GetSeconds();
...
//     // Calculate distance (optional for dataset)
//     Ipv4Address senderIp = address.GetIpv4();
//     if (ipToNodeIdMap.find(senderIp) == ipToNodeIdMap.end()) {
//         NS_LOG_WARN("Sender IP not found in ipToNodeIdMap: " << senderIp);
//         return; // Exit function if sender IP is not mapped
//     }
...
//     // Log details to GPS log file
//     gpsLogFile << receiveTime << ",Received,GPS,"
//                << address.GetIpv4() << ","                // Source Address
//                << baseStationAddress << ","               // Destination Address
//                << receivedPosition.x << ","               // Current X position
//                << receivedPosition.y << ","               // Current Y position
//                << receivedPosition.z << ","               // Current Z position
//                << packet->GetSize() << " bytes (GPS)" << std::endl;
//     //gpsLogFile.flush(); // Ensure immediate write to file

//     // Log packet event with calculated delay and throughput
//     double delay = 0.0;
//     if (sendTimestamps.count(packetId) > 0) {
//         delay = receiveTime - sendTimestamps[packetId];
//     } else {
...
//         gpsPort,
//         "GPS Update Received",    // Packet type
