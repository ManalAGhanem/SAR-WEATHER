...
void SendSignalData(Ptr<Socket> socket, Ipv4Address receiverAddress, uint16_t port, 
  ...
  //  std::cout << "Payload constructed: " << payload << std::endl;
    ...
    // Correctly schedule the next signal
...
//                (Simulator::Now().GetSeconds() - sendTimestamps[packetId]) : -1.0;
// if (delay < 0) {
//     //std::cerr << "Warning: Delay calculation failed for Packet ID: " << packetId << std::endl;
// }
