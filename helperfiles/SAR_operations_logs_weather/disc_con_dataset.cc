//-----------------------discovery and convergence dataset--------------------------------
...
struct DiscoveryConvergenceEvent
{
   ...
   // double totalConvergenceDistance;
                              ...
                              )
{
    ...
    if (g_civilianToEventIdMap.find(civilianId) == g_civilianToEventIdMap.end())
    {
       ...
       // event.discoveringDistances.push_back(distance);
       ...
       // event.totalConvergenceDistance;
            ...
            if (rec.eventId == eventId)
            {
                ...
                for (size_t i = 0; i < discoveringNodeIds.size(); i++)
                {
                    ...
                    if (it == rec.discoveringNodeIds.end())
                    {
                        ...
                        //rec.discoveringDistances.push_back(distance);
...
void RecordConvergenceComplete(
    ...
    for (auto& rec : g_discoveryConvergenceEvents) {
        if (rec.eventId == eventId) {
            ...
            // Update the record with the calculated distance
          //  rec.totalConvergenceDistance = totalDistance;
...
// Mark the completion of rescue phase
...
//         // Optional: if this can be called more than once, skip if already done
//         if (rec.isFinalised) {   // add bool isFinalised to your struct
//             return;
//         }
...
//                 // distance from first recorded SAR position to final rescuer position
//                 totalDistance += ns3::CalculateDistance(
//                     sarPreviousPositions.front(), 
//                     rescuerPosition
//                 );

//                 // distance along the SAR path
//                 for (size_t i = 1; i < sarPreviousPositions.size(); ++i) {
//                     totalDistance += ns3::CalculateDistance(
//                         sarPreviousPositions[i - 1],
//                         sarPreviousPositions[i]
//                     );
//                 }

//                 rec.totalTravelDistanceToBase = totalDistance;
//                 rec.avgRescueDistance = totalDistance / 
//                     static_cast<double>(sarPreviousPositions.size());
//             } 
//             else {
...
//             // distance civilian → base at rescue completion
...
//         // 🔹 (Optional but recommended) call your CSV logger here
//         // LogDiscoveryConvergenceEvent(rec);

//         // 🔹 Now aggressively free heavy per-event vectors if not needed anymore
//         // Replace these with the actual member names in your struct:
//         std::vector<uint32_t>().swap(rec.discoveringNodeIds);
//         std::vector<Ipv4Address>().swap(rec.discoveringNodeIps);
//         std::vector<uint32_t>().swap(rec.convergenceNodeIds);

//         // mark as finalised so we don't process again
//         rec.isFinalised = true;

//         break;  // we found the event, no need to keep looping
//     }
// }
    ...
    for (auto &rec : g_discoveryConvergenceEvents) {
        if (rec.eventId == eventId) {
            ...
            if (rescuerMobility && baseMobility && civilianMobility) {
                ...
                //  FIX: Remove separate "final leg" calculation (this is already covered in `sarPreviousPositions`)
                rec.totalTravelDistanceToBase = totalDistance;

                //  FIX: `avgRescueDistance` should represent **the total movement, not just final SAR position**
                rec.avgRescueDistance = totalDistance / (double)sarPreviousPositions.size();

                //  FIX: `safetyLocation` should use **civilian's original attachment point, not base**
                rec.safetyLocation = ns3::CalculateDistance(civilianPosition, basePosition);
...
while (it != g_discoveryConvergenceEvents.end())
{
    ...
    // // Only write COMPLETED events
    // if (rec.rescueEndTime <= 0.0)
    // {
    //     ++it; // still active, keep in RAM
    //     continue;
    // }
    ...
    // ---------- ERASE COMPLETED EVENT (RAM optimisation) ----------
...
//---------------------end of discovery and convergence dataset----------------------------------------------------
