...
void SendVoiceData(Ptr<Socket> socket, Ipv4Address receiverAddress, uint16_t port, std::string senderNode, double interval) {
    ...
    //     session.totalBytes += packet->GetSize();
    // }


        //   voiceLogFile << sendTime << ",Sent,Voice," << senderAddress << ","
        //          << receiverAddress << "," << packet->GetSize() << " bytes" <<","<<(int)tos<<std::endl;
...
void SetupVoiceCommunication(NodeContainer vehicleNode,NodeContainer Helicopter, NodeContainer footNodes, Ptr<Node> baseNode) {
    // Combine vehicleNode and footNodes into a single NodeContainer for easy management
    // groundNodes.Add(vehicleNode);
    // groundNodes.Add(footNodes);
   // groundNodes.Add(Helicopter);
    ...
    for (uint32_t i = 0; i < groundNodes.GetN(); ++i) {
        ...
        for (uint32_t j = 0; j < groundNodes.GetN(); ++j) {
            if (i != j) { // Avoid sending to self
                ...
                // Generate a unique Communication ID for ground-to-ground communication
                //uint32_t communicationId = 5300 + i ;
                   // Map the sender and receiver IPs to the Communication ID
               // ipToCommIdMap[senderAddress] = communicationId;
                //ipToCommIdMap[receiverAddress] = communicationId;


                // Start the communication session
               // StartCommunicationSession(communicationId, senderAddress, receiverAddress, "GroundToGround Voice Communication", Simulator::Now().GetSeconds(), "UDP",voicePort);
       ...
       // uint32_t groundToBaseCommId = 5200 + i; // Unique Communication ID for ground-to-base

       // StartCommunicationSession(groundToBaseCommId, senderAddress, baseAddress, "GroundToBase Voice Communication", Simulator::Now().GetSeconds(), "UDP",voicePort);
    ...
    for (uint32_t i = 0; i < groundNodes.GetN(); ++i) {
        ...
        // Generate a unique Communication ID for base-to-ground communication
       // uint32_t baseToGroundCommId = 5100 + i;

       // StartCommunicationSession(baseToGroundCommId, baseNode->GetObject<Ipv4>()->GetAddress(1, 0).GetLocal(), receiverAddress, "BaseToGround Voice Communication", Simulator::Now().GetSeconds(), "UDP",voicePort);
